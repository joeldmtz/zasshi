    ...
    $scope.gotoElement = function (id){
     anchorSmoothScroll.scrollTo(id);
       ...
       .controller('menu',function ($scope,$http) {
           $scope.selected = 0;
           $scope.types = []
           $http.get('http://localhost/menus',function (res) {
               alert(res)
           })
           $scope.types[0].subTypes = [
               {
                   name:'ENTRADAS',
                   dishes:[
                       {
